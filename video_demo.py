...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--feature_extractor",
        default="feature_extractor_pretrained/c3d.pickle",
    ...
    parser.add_argument(
        "--ad_model", default="weights/best.pt", help="path to the trained AD model"
    )
    parser.add_argument(
        "--n_segments",
        type=int,
...
def ad_prediction(model: AnomalyDetector, features: Tensor, device: Device) -> Tensor:
    ...
    # tensor_min = preds.min()
    # tensor_max = preds.max()
    # preds = (preds - tensor_min) / (tensor_max - tensor_min)
