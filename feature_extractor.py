...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--save_dir",
        type=str,
        default="dataset/features/gradel",
    ...
    parser.add_argument(
        "--pretrained_3d", type=str,default="feature_extractor_pretrained/c3d.pickle", help="load default 3D pretrained model."
    )
...
def to_segments(
    data: Union[Tensor, np.ndarray], n_segments: int = 64
) -> List[np.ndarray]:
...
class FeaturesWriter:
    ...
    def dump(self) -> None:
        ...
        """
        logging.info(f"{self.dump_count+1} / {self.num_videos}:	Dumping {self.path}")
        self.dump_count += 1
...
#serve to Annotation.txt saving
def list_files_in_event_folders(main_folder_path):
    # List all folders in the main folder
    event_folders = [folder for folder in os.listdir(main_folder_path) if os.path.isdir(os.path.join(main_folder_path, folder))]
    ...
    for event_folder in event_folders:
        event_folder_path = os.path.join(main_folder_path, event_folder)

        # List all files in the event folder and get their paths
        files_in_event = [os.path.join(event_folder_path, file) for file in os.listdir(event_folder_path) if os.path.isfile(os.path.join(event_folder_path, file))]

        # Append the file paths to the list
        all_file_paths.extend(files_in_event)
...
if __name__ == "__main__":
        ...
        for data, clip_idxs, dirs, vid_names in data_iter:
            ...
            for i, (_dir, vid_name, clip_idx) in enumerate(
                ...
                if loop_i == 0:
                    ...
                    logging.info(
                        f"Video {features_writer.dump_count+1} / {features_writer.num_videos} : Writing clip {clip_idx} of video {vid_name}"
                    )
    ...
    main_folder_path = f"{args.save_dir}"  # feature folder path
    file_paths = list_files_in_event_folders(main_folder_path) #get the list of feature paths in feature folder

    txt_file_folder = "dataset/annotations/gradel"
    txt_file_name = "Training_Annotations.txt"
    txt_file_path = f"{txt_file_folder}/{txt_file_name}"
    os.makedirs(txt_file_folder, exist_ok=True)
    with open(txt_file_path, "w") as txt_file:
        for file_path in file_paths:
            txt_file.write(file_path + "\n")

    print(f"Annotation file saved to: {txt_file_path}")
